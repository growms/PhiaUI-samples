defmodule PhiaDemoWeb.Demo.Todo.Layout do
  @moduledoc "Todo app shell layout."
  ...
  def layout(assigns) do
    ...
    <.shell>
      <:topbar>
        ...
        <.project_topbar current_project={:todo} dark_mode_id="todo-dm" />
      </:topbar>
      <:sidebar>
        <.sidebar>
          <:brand>
            <div class="flex items-center gap-2.5">
              ...
              <div>
                <span class="text-sm font-bold text-foreground leading-none">Todo</span>
                <p class="text-[10px] text-muted-foreground leading-none mt-0.5 font-medium">Task Manager</p>
              </div>
            </div>
          </:brand>
          <:nav_items>
            <.nav_section label="Lists">
              <.nav_item current_path={@current_path} href="/todo" icon="list" label="All Tasks" badge={8} />
              <.nav_item current_path={@current_path} href="/todo" icon="zap" label="Today" badge={3} />
              <.nav_item current_path={@current_path} href="/todo" icon="star" label="Starred" />
              <.nav_item current_path={@current_path} href="/todo" icon="circle-check" label="Completed" />
            </.nav_section>
            <.nav_section label="Projects">
              <.nav_item current_path={@current_path} href="/todo" icon="tag" label="Work" />
              <.nav_item current_path={@current_path} href="/todo" icon="tag" label="Personal" />
              <.nav_item current_path={@current_path} href="/todo" icon="tag" label="Shopping" />
            </.nav_section>
          </:nav_items>
          <:footer_items>
            <div class="px-3 py-1">

defmodule PhiaDemoWeb.Demo.Mail.Layout do
  @moduledoc "Mail app shell layout."
  ...
  def layout(assigns) do
    ...
    <.shell>
      <:topbar>
        ...
        <.project_topbar current_project={:mail} dark_mode_id="mail-dm" />
      </:topbar>
      <:sidebar>
        <.sidebar>
          <:brand>
            <div class="flex items-center gap-2.5">
              ...
              <div>
                <span class="text-sm font-bold text-foreground leading-none">Mail</span>
                <p class="text-[10px] text-muted-foreground leading-none mt-0.5 font-medium">Inbox App</p>
              </div>
            </div>
          </:brand>
          <:nav_items>
            <.nav_section label="Mailbox">
              <.nav_item current_path={@current_path} href="/mail" icon="inbox" label="Inbox" badge={4} />
              <.nav_item current_path={@current_path} href="/mail" icon="send" label="Sent" />
              <.nav_item current_path={@current_path} href="/mail" icon="pencil" label="Drafts" badge={2} />
              <.nav_item current_path={@current_path} href="/mail" icon="trash-2" label="Trash" />
            </.nav_section>
            <.nav_section label="Labels">
              <.nav_item current_path={@current_path} href="/mail" icon="tag" label="Work" />
              <.nav_item current_path={@current_path} href="/mail" icon="tag" label="Personal" />
              <.nav_item current_path={@current_path} href="/mail" icon="star" label="Starred" />
            </.nav_section>
          </:nav_items>
          <:footer_items>
            <div class="px-3 py-1">

defmodule PhiaDemoWeb.Demo.Courses.Layout do
  @moduledoc "Courses app shell layout."
  ...
  def layout(assigns) do
    ...
    <.shell>
      <:topbar>
        ...
        <.project_topbar current_project={:courses} dark_mode_id="courses-dm" />
      </:topbar>
      <:sidebar>
        <.sidebar>
          <:brand>
            <div class="flex items-center gap-2.5">
              ...
              <div>
                <span class="text-sm font-bold text-foreground leading-none">Courses</span>
                <p class="text-[10px] text-muted-foreground leading-none mt-0.5 font-medium">Learning Platform</p>
              </div>
            </div>
          </:brand>
          <:nav_items>
            <.nav_section label="Learning">
              <.nav_item current_path={@current_path} href="/courses" icon="layers" label="Catalog" />
              <.nav_item current_path={@current_path} href="/courses" icon="circle-check" label="My Courses" badge={3} />
              <.nav_item current_path={@current_path} href="/courses" icon="star" label="Bookmarked" />
              <.nav_item current_path={@current_path} href="/courses" icon="trending-up" label="Progress" />
            </.nav_section>
            <.nav_section label="Topics">
              <.nav_item current_path={@current_path} href="/courses" icon="zap" label="Elixir" />
              <.nav_item current_path={@current_path} href="/courses" icon="zap" label="Phoenix" />
              <.nav_item current_path={@current_path} href="/courses" icon="zap" label="LiveView" />
            </.nav_section>
          </:nav_items>
          <:footer_items>
            <div class="px-3 py-1">

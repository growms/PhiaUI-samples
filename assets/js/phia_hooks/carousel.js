...
const PhiaCarousel = {
  mounted() {
    ...
    this._handleKeydown = (e) => {
      if (this._orientation === "horizontal") {
        if (e.key === "ArrowLeft") { e.preventDefault(); this._goTo(this._currentIndex - 1); }
        else if (e.key === "ArrowRight") { e.preventDefault(); this._goTo(this._currentIndex + 1); }
      } else {
        if (e.key === "ArrowUp") { e.preventDefault(); this._goTo(this._currentIndex - 1); }
        else if (e.key === "ArrowDown") { e.preventDefault(); this._goTo(this._currentIndex + 1); }
      }
    };
    ...
    this._handleTouchEnd = (e) => {
      ...
      } else if (this._orientation === "vertical" && Math.abs(dy) > threshold) {
  ...
  _updateButtons() {
    ...
    const count = this._items.length;
    if (this._prevBtn) this._prevBtn.disabled = this._currentIndex === 0;
    if (this._nextBtn) this._nextBtn.disabled = this._currentIndex === count - 1;
  },

  destroyed() {
    if (this._prevBtn && this._handlePrev) this._prevBtn.removeEventListener("click", this._handlePrev);
    if (this._nextBtn && this._handleNext) this._nextBtn.removeEventListener("click", this._handleNext);
